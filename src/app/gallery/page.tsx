...
import { roof_replacement_slides } from "@/components/GalleryData";
  ...
  return (
    <div className="w-full h-auto flex flex-col items-center justify-center rounded-2xl border-2 border-solid border-matteblack bg-coconut p-6 lg:p-4 md:p-6 relative xs:p-4">
      ...
      <div className="w-full flex flex-col items-center justify-center mt-4">
        <span
          className={`text-customMustardYellow text-3xl xl:text-2xl lg:text-xl md:text-4xl sm:text-3xl xs:text-2xl xxxs:text-xl ${mondaFont.className}`}
        >
          {title}
        </span>
      </div>
    ...
    <main className="mt-24 overflow-x-hidden">
          ...
          <div className="w-full h-full inline-block z-0 px-16 xl:px-12 lg:px-8 sm:px-12 xs:px-10 xxs:px-8">
            <div className="grid grid-cols-12 gap-x-16 gap-y-16 xl:gap-x-12 lg:gap-x-8 md:gap-x-0 md:gap-y-20 sm:gap-y-16">
              ...
              <div className="col-span-4 md:col-span-12">
                <GalleryComponent
                  imgSrc={roof_replacement_slides}
                  title="Roof Replacement"
                  coverPhoto={roof_replacement_cover_photo.src}
                />
              </div>
      ...
      <section className={styles.galleryImageOne}>
        <div className="flex flex-col text-center 2xl:px-10 sm:px-5">
          <h1
            className={`uppercase text-ivory break-words text-5xl xl:text-4xl lg:text-3xl md:text-4xl sm:text-3xl xs:text-2xl xxxs:text-xl ${robotoFont.className}`}
          >
            start your next project today with Shumba Carpenters - Expert
            Roofing Solutions
          </h1>
          ...
          <h2
            className={`pb-5 text-ivory break-words text-3xl xl:text-2xl lg:text-xl sm:text-lg xs:text-base xxxs:text-sm`}
          >
            Transform your dream home into reality by connecting with our
            experts today&#33;
          </h2>
